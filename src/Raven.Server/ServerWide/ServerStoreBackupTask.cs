...
namespace Raven.Server.ServerWide
{
    public class ServerStoreBackupTask
    {
        ...
        public ServerStoreBackupTask(
            ...
            Logger logger,
            LocalSettings localSettings,// todo: remove that, it is temporary
        ...
        public IOperationResult RunPeriodicBackup(Action<IOperationProgress> onProgress)
        {
            ...
            try
            {
                ...
                // todo: decide whether we want to implement periodic backup (apparently not)
                ...
                //    var currentLastEtag = _serverStore.LastRaftCommitIndex;
                ...
                //        //DatabaseSmuggler.EnsureProcessed(_backupResult); todo: do we want something like that?
                //        AddInfo(message);
                ...
                //todo: track server store backup progress
                ...
                LocalSettings localSettings = _localSettings;
        ...
        private static string GetBackupExtension(BackupType type, bool isFullBackup, bool isEncrypted)
        {
            if (isFullBackup == false)
                return isEncrypted ? Raven.Client.Constants.Documents.PeriodicBackup.EncryptedIncrementalBackupExtension :
                    Raven.Client.Constants.Documents.PeriodicBackup.IncrementalBackupExtension;

            switch (type)
            {
                case BackupType.Backup:
                    return isEncrypted ?
                        Raven.Client.Constants.Documents.PeriodicBackup.EncryptedFullBackupExtension : Raven.Client.Constants.Documents.PeriodicBackup.FullBackupExtension;
                case BackupType.Snapshot:
                    return isEncrypted ?
                        Raven.Client.Constants.Documents.PeriodicBackup.EncryptedSnapshotExtension : Raven.Client.Constants.Documents.PeriodicBackup.SnapshotExtension;
                default:
            ...
            PeriodicBackupStatus status, string backupFilePath, long? startDocumentEtag, long? startRaftIndex)
        {
            ...
            using (status.LocalBackup.UpdateStats(_isFullBackup))
            {
                try
                {
                    ...
                    //    {
                    //        AuthorizationStatus = AuthorizationStatus.ClusterAdmin,
                    //    };
                    //    //if (_isFullBackup == false)
                    //    //{
                    ...
                    internalBackupResult.LastRaftIndex = GetServerStoreRaftIndex();
        ...
        //    using (_serverStore.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
        //    {
