    ...
    public class Subscriptions : RavenTestBase
    {
        ...
        public async Task CreateSubscription()
        {
            using (var store = await GetDocumentStore())
            {
                ...
                // open subscription
                var subscriptionConnectionOptions = new Raven.Abstractions.Data.SubscriptionConnectionOptions();
                var subscription =  subscriptionManager.Open(subsId, subscriptionConnectionOptions);
                

                HttpJsonRequest request = subscription.GetType().GetMethod("CreateClientAliveRequest", BindingFlags.NonPublic | BindingFlags.Instance).Invoke(subscription, null) as HttpJsonRequest;
                request.ExecuteRequest();
                subscriptionsConfig = subscriptionManager.GetSubscriptions(0, 10);
                var now = DateTime.UtcNow;
                Assert.InRange<DateTime>(new DateTime(subscriptionsConfig[0].TimeOfLastClientActivity), now.AddHours(-1), now);
